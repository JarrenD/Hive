...
class Hive(object):
    ...
    def action(self, actionType, action):
        ...
        #print(f"\nIt's {player}'s turn.")
        self.find_valid_moves(player)  # Print all possible moves for the player
...
# ---
    ...
    def find_valid_moves(self, player):
        ...
        Includes both piece placements and moves of already played pieces.
        """
        valid_moves = []

        # 1. Check all unplayed pieces for valid placements
        for piece_name, piece in self.unplayedPieces.get(player, {}).items():
            if self.turn == 1:
                try:
                    if self._validate_turn(piece, 'place') and self._validate_place_piece(piece, (0, 0)):
                        valid_moves.append(('play', (piece_name, None, None)))
                except Exception:
                    pass
            else:
                for ref_piece_name in self.playedPieces:
                    for direction in [self.O, self.W, self.NW, self.NE, self.E, self.SE, self.SW]:
                        try:
                            target_cell = self._poc2cell(ref_piece_name, direction)
                            # Defensive: skip if target_cell is None or already occupied by this piece
                            if target_cell is None:
                                continue
                            # Defensive: skip if this piece is already at target_cell
                            if piece_name in self.piecesInCell.get(target_cell, []):
                                continue
                            if self._validate_turn(piece, 'place') and self._validate_place_piece(piece, target_cell):
                                # --- Simulate placement for one hive rule ---
                                # Temporarily place the piece
                                self.playedPieces[piece_name] = {'piece': piece, 'cell': target_cell}
                                pic = self.piecesInCell.setdefault(target_cell, [])
                                pic.append(piece_name)
                                breaks_one_hive = not self._one_hive(piece)
                                # Undo the temporary placement
                                pic.remove(piece_name)
                                if not pic:
                                    del self.piecesInCell[target_cell]
                                del self.playedPieces[piece_name]
                                # Only add if it doesn't break one hive rule
                                if not breaks_one_hive:
                                    valid_moves.append(('play', (piece_name, ref_piece_name, direction)))
                        except Exception:
                            continue

        # 2. Check all played pieces for valid moves
        for piece_name, info in self.playedPieces.items():
            piece = info['piece']
            if piece.color != player:
                continue
            start_cell = info['cell']
            # Only top piece in stack can move
            if self.piecesInCell[start_cell][-1] != piece_name:
                continue
            for ref_piece_name in self.playedPieces:
                for direction in [self.O, self.W, self.NW, self.NE, self.E, self.SE, self.SW]:
                    try:
                        target_cell = self._poc2cell(ref_piece_name, direction)
                        # Defensive check: skip if target_cell is None or same as start_cell
                        if target_cell is None or target_cell == start_cell:
                            continue
                        # Defensive: skip if this piece is already at target_cell
                        if piece_name in self.piecesInCell.get(target_cell, []):
                            continue
                        if self._validate_turn(piece, 'move') and self._validate_move_piece(piece, target_cell):
                            # Simulate move for one hive rule
                            self.piecesInCell[start_cell].remove(piece_name)
                            pic = self.piecesInCell.setdefault(target_cell, [])
                            pic.append(piece_name)
                            self.playedPieces[piece_name]['cell'] = target_cell
                            breaks_one_hive = not self._one_hive(piece)
                            # Undo the temporary move
                            pic.remove(piece_name)
                            if not pic:
                                del self.piecesInCell[target_cell]
                            self.piecesInCell[start_cell].append(piece_name)
                            self.playedPieces[piece_name]['cell'] = start_cell
                            if not breaks_one_hive:
                                valid_moves.append(('move', (piece_name, ref_piece_name, direction)))
                    except Exception:
                        continue

        # Final filtering step to ensure no move is to the same cell or is otherwise invalid
        filtered_moves = []
        for move in valid_moves:
            if move[0] == 'move':
                piece_name, ref_piece_name, direction = move[1]
                # Don't allow moving with self as reference
                if piece_name == ref_piece_name:
                    #print(f"Filtered out move with self as reference: {move}")
                    continue
                start_cell = self.locate(piece_name)
                target_cell = self._poc2cell(ref_piece_name, direction)
                if start_cell == target_cell:
                    #print(f"Filtered out move-to-same-cell: {move}")
                    continue
            elif move[0] == 'play':
                piece_name, ref_piece_name, direction = move[1]
                # Don't allow playing a piece that's already on the board
                if piece_name in self.playedPieces:
                    #print(f"Filtered out play of already played piece: {move}")
                    continue
                # Don't allow playing to an occupied cell
                target_cell = (0, 0) if ref_piece_name is None else self._poc2cell(ref_piece_name, direction)
                if not self._is_cell_free(target_cell):
                    #print(f"Filtered out play to occupied cell: {move}")
                    continue
