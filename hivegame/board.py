...
def get_board_state_and_reserves(hive):
    """
    Extracts the current board state and reserves from the Hive object.

    Returns:
        board_state: list of (q, r, stack) where stack is a list of piece strings.
        reserves: dict {(color, piece_type): count}
    """
    board_state = []
    reserves = {('w', p): 0 for p in ['Q', 'A', 'S', 'G', 'B']}
    reserves.update({('b', p): 0 for p in ['Q', 'A', 'S', 'G', 'B']})

    # --- Board State ---
    for cell, stack in hive.board.items():
        q, r = cell.q, cell.r
        stack_list = []
        for piece in stack:
            color = piece.color[0].lower()  # 'w' or 'b'
            ptype = piece.piece.upper()     # 'Q', 'A', etc.
            stack_list.append(f"{color}{ptype}")
        board_state.append((q, r, stack_list))

    # --- Reserves ---
    # Your Hive object should have some way to know what pieces are unplaced.
    # If not, we calculate from full piece set minus what's on board.
    def count_unplaced(color):
        from hivegame.piece import HivePiece
        piece_pool = []
        for i in range(3):
            piece_pool.append(HivePiece(color, 'A', i+1))
            piece_pool.append(HivePiece(color, 'G', i+1))
        for i in range(2):
            piece_pool.append(HivePiece(color, 'S', i+1))
            piece_pool.append(HivePiece(color, 'B', i+1))
        piece_pool.append(HivePiece(color, 'Q', 1))
        return piece_pool

    for color in ['w', 'b']:
        all_pieces = count_unplaced(color)
        placed = [p for stack in hive.board.values() for p in stack if p.color[0].lower() == color]
        unplaced = [p for p in all_pieces if p not in placed]
        for p in unplaced:
            reserves[(color, p.piece.upper())] += 1
